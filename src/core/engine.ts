...
export class Engine {
    ...
    executors: Executor<Actions>[] = [];
    strategies: Strategy<Collectors, Actions>[] = [];

    constructor(public readonly stream: EventEmitter = new EventEmitter()) {}
    ...
    run = async () => {
        ...
        const collectorsPromises = this.collectors.map(
            async (collector) => await collector.getEventStream(this.stream),
        );
