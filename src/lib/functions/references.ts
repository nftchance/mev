...
const generateStaticReferences = async ({
    ...
    let protocolGeneration = `
        // Autogenerated file for ${key} through "mev references". Do not edit directly.
...
>(
    ...
    responses = responses
        ...
        .filter((response) => response.abi !== undefined)
    ...
    await Promise.all(
        responses.map(
            async ({ key, name, address, abi, bytecode, deployedBytecode }) => {
                ...
                if (abi === undefined) {
                    logger.error(`Failed to retrieve ABI for ${name}.`)
                    return
                }
