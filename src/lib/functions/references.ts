...
const generateStaticReference = async (reference: StaticReference) => {
    ...
    const imports = [address === undefined ? "Interface" : "Contract"]
    ...
    if (abi) {
        ...
        if (address !== undefined)
            ...
            contractInterface += dedent`\n
                export const ${bigName}_INTERFACE = new Interface(
                    ${bigName}_ABI
                )
            `

        /// * Create a record of all the event topics for the contract.
        const protocolInterface = new Interface(abi)
        const eventTopics: Record<string, string> = {}
        const abiEvents = JSON.parse(abi)
            .filter((x: any) => x.type === "event")
            .entries()

        for (let [_, event] of abiEvents) {
            const topicHash = protocolInterface.getEvent(event.name)?.topicHash

            if (topicHash !== undefined) eventTopics[event.name] = topicHash
        }
